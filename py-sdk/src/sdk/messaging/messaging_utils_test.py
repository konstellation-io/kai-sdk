from messaging.messaging_utils import MessagingUtils, GZIP_HEADER, FailedGettingMaxMessageSizeError, size_in_mb, size_in_kb, compress, decompress, is_compressed
...
async def test_get_max_message_size_ok(nats_max_payload_mock, jetstream_context_mock, nats_max_payload, jetstream_max_msg_size, max_message_size):
    v.set("nats.stream", "test_stream")
    nc = NatsClient()
    js = nc.jetstream()
    nats_max_payload_mock.return_value = nats_max_payload
    jetstream_context_mock.return_value = StreamInfo(config=StreamConfig(max_msg_size=jetstream_max_msg_size), state=None)
