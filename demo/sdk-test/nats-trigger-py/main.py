...
async def process_message(trigger_runner: trigger_runner.TriggerRunner, sdk: sdk.KaiSDK, msg: Msg):
...
async def nats_subscriber_runner(trigger_runner: trigger_runner.TriggerRunner, sdk: sdk.KaiSDK):
    ...
    nats_url = await sdk.centralized_config.get_config("nats.url")
    logger.info(f"nats url: {nats_url[0]}")
    nc: NatsClient = NatsClient()
    js: JetStreamContext = nc.jetstream()
    await nc.connect(nats_url[0])
